 ...
 *          Defaults:  2      0        0         0
 * 
 * The command-line arguments are the second and microsecond settings for the 
 * timer's initial value and interval.
 * 
 ...
 * initializatin of the latter variable. See the erratum for page 983 to 984.
*/
#include <signal.h>
#include <sys/time.h>
#include <time.h>
 ...
 * TRUE) the current value an interval for the ITIMER_REAL timer.
*/
static void
displayTimes(const char *msg, Boolean includeTimer)
{
    ...
    if(gettimeofday(&curr, NULL) == -1)
        errExit("gettimeofday");
    printf("%-7s %6.6f", msg, curr.tv_sec - start.tv_sec + 
...
main(int argc, char *argv[])
{
    ...
    // Set timer from the commane-line arguments
    ...
    while (1)
    {
        // Inner loop consums at least 0.5 seconds CPU time
